...
def main():
    ...
    df = encode_label_to_close_int(df,''.join(HVR_dir+'/genus_mapping.csv'),
                                   ''.join(HVR_dir+'/species_mapping.pkl'),encode_species = False)
    ...
    train,test,_,_ = train_test_split(df,df['genus-'],random_state= 1,test_size = 0.2)
    train,valid,_,_ = train_test_split(train,train['genus-'],random_state= 1,test_size = 0.15)
    ...
    df = df.drop(columns=['ambiguity_count'])
    df = df.drop(columns=['phylum-', 'class_-', 'order-', 'family-','genus-'])
    """
    #Saving the phylogenetic tree as list of lists
    tree = build_tree(df)
    with open(database+'/tree.pkl', 'wb') as f:
        pickle.dump(tree, f)
    """
